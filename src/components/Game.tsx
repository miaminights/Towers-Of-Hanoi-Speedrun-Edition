...
export const Game = () => {
  ...
  const resetGame = useCallback((newSettings: Settings) => {
    ...
    stacks[Math.min(newSettings.startStack, newSettings.stacks) - 1] =
      Array.from({ length: newSettings.disks }, (_, i) => i + 1);
    setStacks(stacks);
  ...
  const onStackClick = (stackIndex: number) => {
    if (holding) {
      if (!settings.illegalMoves && stacks[stackIndex][0] < holding.width)
        return;

      const newStacks = [...stacks];
      newStacks[holding.from].shift();
      newStacks[stackIndex].unshift(holding.width);
      setStacks(newStacks);
      setHolding(
        stacks[stackIndex][0]
          ? {
              width: stacks[stackIndex][0],
              from: stackIndex,
            }
          : null
      );
      ...
      if (holding.from !== stackIndex)
        setMoves(moves.concat([{ from: holding.from, to: stackIndex }]));
      checkForWin();
    } else {
      setHolding(
        stacks[stackIndex][0]
  ...
  const onKeyDown = (event: KeyboardEvent) => {
    ...
    if (holding) {
      if (!settings.illegalMoves && stacks[numberKey][0] < holding.width)
        return;
      ...
      if (holding.from !== numberKey)
        setMoves(moves.concat([{ from: holding.from, to: numberKey }]));
      checkForWin();
    } else if (stacks[numberKey][0]) {
      setHolding({
        width: stacks[numberKey][0],
        from: numberKey,
      });
  ...
  const getWidth = (size: number) => {
    ...
    const maxWidth = (stacksRef.current.firstChild as HTMLDivElement)
      .offsetWidth;
  ...
  const diskHeight = Math.min(
    (window.innerHeight - TOP_DISK_MARGIN) / settings.disks,
    MAX_DISK_HEIGHT
  );
  ...
  return (
    <>
      ...
      />
      {infoShown && (
        <Info onClose={() => setInfoShown(false)} settings={settings} />
      )}
      {settingsShown && (
        <SettingsMenu
          settings={settings}
        ...
        <Stacks ref={stacksRef}>
          {stacks.map((stack, key) => (
            <Stack
              ...
              isHolding={holding?.from === key}
              diskHeight={diskHeight}
              onClick={() => onStackClick(key)}
            >
              {stack.map((index, key) => (
                <Disk
                  key={key}
